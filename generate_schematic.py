...
with open('src/components/LivePulseSchematic.tsx', 'r') as f:
    code = f.read()

regex = r'\{\/\* ZONAL RINGS \*\/\}[\s\S]*?\{\/\* Draw connecting route if active \*\/\}'
...
replaced = re.sub(regex, newCode, code)
with open('src/components/LivePulseSchematic.tsx', 'w') as f:
    f.write(replaced)
