...
interface LivePulseSchematicProps {
  zones: Zone[];
  onZoneClick?: (zoneId: string) => void;
  selectedSourceZone?: string;
  selectedDestZone?: string;
}

export default function LivePulseSchematic({ zones, onZoneClick, selectedSourceZone, selectedDestZone }: LivePulseSchematicProps) {
  return (
    <svg viewBox="0 0 400 400" className="w-full h-auto max-w-full aspect-square">
      {/* Outermost border */}
      <circle cx="200" cy="200" r="185" fill="none" stroke="var(--color-chalk)" strokeWidth="2" opacity="0.2" />
      {/* Stadium pitch representation */}
      <rect x="140" y="120" width="120" height="160" rx="6" fill="var(--color-pitch)" stroke="var(--color-live-amber)" strokeWidth="1" opacity="0.4" />
      <line x1="140" y1="200" x2="260" y2="200" stroke="var(--color-live-amber)" strokeWidth="1" opacity="0.4" />
      <circle cx="200" cy="200" r="28" fill="none" stroke="var(--color-live-amber)" strokeWidth="1" opacity="0.4" />
      ...
      {selectedSourceZone && selectedDestZone && (
        <path
          d="M 200 110 C 270 130, 270 270, 200 290"
          fill="none"
